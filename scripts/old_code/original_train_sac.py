...
TRAIN_CAMERA = {'placement_kwargs': {'position':[   0., -150.,  140.], 'orientation': [0.48480962, 0.        , 0.        , 0.87461971], 'lookAt': [ 0., 10.,  0.]}}
...
BASE_POS   = np.array([0, -150, 140.0])
BASE_LOOK  = np.array([0,  10,  0])
...
delta_mm = 50.0
translations = {
    f"shift+x{sign:+d}50": {"placement_kwargs": {"position": (BASE_POS + np.array([ sign*delta_mm, 0, 0])),
                         "lookAt":  BASE_LOOK}}
    for sign in (+1,)
}
translations |= {
        f"shift+y{sign:+d}50": {"placement_kwargs": {"position": (BASE_POS + np.array([0, sign*delta_mm, 0])),
                         "lookAt":  BASE_LOOK}}
    for sign in (+1,)
}

translations |= {
        f"shift+z{sign:+d}50": {"placement_kwargs": {"position": (BASE_POS + np.array([0, 0, sign*delta_mm])),
...
# ------------------------------------------------------------
# rolls = {
#     f"roll{d:+d}deg": {"placement_kwargs": {camera_with_roll(BASE_POS, BASE_LOOK, d) for d in (+15, +30,)
#     }}
# }

rolls = {}
for d in (+15, -30,):
    rolls[f"roll{d:+d}deg"] = {"placement_kwargs": camera_with_roll(BASE_POS, BASE_LOOK, d)}
...
# ------------------------------------------------------------
handcrafted = {
    "nominal":  {"placement_kwargs": {"position": BASE_POS, "lookAt": BASE_LOOK}},
...
# ------------------------------------------------------------
fovs = {
    "fov30": {"placement_kwargs": {"position": BASE_POS, "lookAt": BASE_LOOK}, "vertical_field_of_view": 30},
    "fov70": {"placement_kwargs": {"position": BASE_POS, "lookAt": BASE_LOOK}, "vertical_field_of_view": 70},
}
...
def build(config: DictConfig) -> Iterator[RLRunner]:
    ...
    with open_dict(config.env):
        ...
        config.env.create_scene_kwargs = {}
        config.env.create_scene_kwargs.camera_config = {}
        config.env.create_scene_kwargs["camera_config"] = TRAIN_CAMERA
    ...
    for name, cam_cfg in EVAL_CAMERAS.items():
        ...
        with open_dict(eval_cfg):
            eval_cfg.env.create_scene_kwargs["camera_config"] = cam_cfg
            eval_cfg.env.mode = "eval"

        base_eval_factory = instantiate(eval_cfg.env, _convert_="partial", _partial_=True)

        cages_i, meta_i = build_cages(
            EnvClass=base_eval_factory,
            n_envs=eval_cfg.eval.n_eval_envs,
        ...
        # clone the template
        eval_tree = eval_tree_example.new_array(batch_shape=(1, eval_cfg.eval.n_eval_envs))
        eval_tree["env_info"] = dict_map(
            Array.from_numpy,
            meta_i.example_info,
            batch_shape=(1, eval_cfg.eval.n_eval_envs),
            storage=storage,
        )
        ...
        if name == "nominal":          # record only one setting
            recorder = RecordVectorizedVideo(
                ...
                env_fps=50,
                output_dir=Path(eval_cfg.video_path) / name,
                video_length=eval_cfg.env.max_episode_steps,
                use_wandb=True,
            )
        ...
        eval_samplers[name] = EvalSampler(
            max_traj_length   = eval_cfg.env.max_episode_steps,
            max_trajectories  = eval_cfg.eval.max_trajectories,
            envs              = cages_i,
            agent             = agent,
            sample_tree       = eval_tree,
