...
TRAIN_POSE = _lookat_quat(TRAIN_POS, TRAIN_LOOK, roll_deg=0.0)
...
translations = {
    "shift+x+50": {
        "position": (BASE_POS + np.array([+delta_mm, 0, 0])).tolist(),
        "lookAt": BASE_LOOK.tolist(),
    },
    "shift+y+50": {
        "position": (BASE_POS + np.array([0, +delta_mm, 0])).tolist(),
        "lookAt": BASE_LOOK.tolist(),
    },
    "shift+z+50": {
        "position": (BASE_POS + np.array([0, 0, +delta_mm])).tolist(),
        "lookAt": (BASE_LOOK + np.array([0, 0, +delta_mm])).tolist(),
    },
}
...
handcrafted = {
    "nominal": {"position": BASE_POS.tolist(), "lookAt": BASE_LOOK.tolist()},
}
...
EVAL_CAMERAS: dict[str, dict] = {
    **handcrafted,
    **translations,
    **fwd_back,
    **rolls,
    **fovs,  # default often ~62 deg
}
...
def _get_named_camera(env, name: str | None):
    """
    Return the requested camera if present.
    If name is None and exactly one camera exists, return that one.
    Otherwise raise with a helpful list of available names.
    """
    u = env.unwrapped
    found_maps = []  # (attr_name, keys_list, dict)
    ...
    if name is not None:
        for attr, keys, d in found_maps:
            if name in d:
                return d[name]

    # If no name specified OR not found: pick the only one if unique
...
class FixedOrConfiguredCamera(gym.Wrapper):
    """
    Train: put the named camera at a fixed Pose (constant viewpoint).
    Eval:  apply a provided camera config per cage (pose + vfov if supported).
    """

    def __init__(
        self,
        env,
        camera_name: str | None = None,  # <-- ensure this matches your YAML camera name
        mode: str = "train",
    ...
    def _apply_pose(self, cam, pose: Pose):
        ...
        for subname in ("camera", "_camera", "sensor", "entity", "cam", "component"):
            ...
            if sub is not None and try_set(sub):
                return
        raise RuntimeError("Camera object lacks usable pose setters (tried pose and extrinsic APIs).")

    # def _apply_pose(self, cam, pose: Pose):
    #     if hasattr(cam, "set_local_pose"):
    #         cam.set_local_pose(pose)
    #     elif hasattr(cam, "set_pose"):
    #         cam.set_pose(pose)
    #     else:
    #         raise RuntimeError("Camera object lacks pose setters.")

    def _apply_pose(self, cam, pose: Pose):
        """
        Try multiple APIs across ManiSkill/SAPIEN variants to set camera pose.
        Falls back to sub-objects like `.camera`, `._camera`, `.sensor`, etc.
        """
        M = pose.to_transformation_matrix()
            ...
            for name, arg in (("set_local_pose", pose), ("set_pose", pose)):
                if hasattr(obj, name):
                    try:
                        ...
                        pass
            # Matrix-based setters (cam-to-world extrinsic)
            for name in ("set_extrinsic", "set_cam2world", "set_extrinsic_cv", "set_world_pose", "set_model_matrix"):
                if hasattr(obj, name):
                    try:
                        getattr(obj, name)(M)
        ...
        # 1) Try the camera wrapper itself
        if try_set(cam):
            return

        # 2) Try common sub-objects where the real camera lives
        for subname in ("camera", "_camera", "sensor", "entity", "cam", "component"):
            sub = getattr(cam, subname, None)
            if sub is not None and try_set(sub):
                return
        ...
        # 3) Nothing worked
        raise RuntimeError("Camera object lacks usable pose setters (tried pose and extrinsic APIs on cam and sub-objects).")


    def _apply_vfov_if_supported(self, cam, vfov_deg: float):
        # Try common API names across ManiSkill/SAPIEN builds.
        for key in ("set_fov_y", "set_fov", "set_vertical_fov", "set_parameters"):
            if hasattr(cam, key):
                try:
                    getattr(cam, key)(vfov_deg)
                    return
                except Exception:
                    pass
...
class TrainCamWrapperFactory:
    """
    Pickle-friendly callable that builds a ManiSkill env and wraps it with a fixed camera.
    We store only primitives; we recompute the Pose on __call__ to avoid pickling SAPIEN objects.
    """
    def __init__(self, base_factory, camera_name, train_pos, train_look, roll_deg=0.0):
        import numpy as _np
        self.base_factory = base_factory
        self.camera_name  = camera_name  # can be None; wrapper will pick unique camera if only one exists
        # store as plain lists (pickle-friendly)
...
class EvalCamWrapperFactory:
    """
    Pickle-friendly callable that builds a ManiSkill env and wraps it with a given eval camera config.
    """
...
def build(config: DictConfig) -> Iterator[RLRunner]:
    ...
    with open_dict(config.env):
        config.env.pop("name")
        traj_info = config.env.pop("traj_info")
        cam_name = config.env.pop("camera_name", None)  # <-- NEW: remove from kwargs passed to env
    ...
    env_factory = instantiate(config.env, _convert_="partial", _partial_=True)
        ...
        # Build pickle-friendly factories (top-level callables)
    train_factory = TrainCamWrapperFactory(
        base_factory=env_factory,
        camera_name=cam_name,                # None or a string like "overhead_camera_0"
        train_pos=TRAIN_POS,                 # uses your constants above
        train_look=TRAIN_LOOK,
        roll_deg=0.0,
    )

# TRAIN cages (process-safe)
    ...
    # cam_name = getattr(config.env, "camera_name")

    # # Wrap factories: fixed train pose, and per-eval config
    # def EnvTrainWithFixedCam(*args, **kwargs):
    #     env = env_factory(*args, **kwargs)
    #     return FixedOrConfiguredCamera(
    #         env,
    #         camera_name=cam_name,  # <-- ensure this matches the camera defined in your YAML
    #         mode="train",
    #         train_pose=TRAIN_POSE,
    #         eval_cam_cfg=None,
    #     )
    #
    # def EnvEvalWithCamCfg(cam_cfg: dict):
    ...
    #             mode="eval",
    #             train_pose=None,
    #             eval_cam_cfg=cam_cfg,
    #         )
    #     return _factory

    # # TRAIN cages (always same viewpoint)
    # cages, metadata = build_cages(
    #     EnvClass=EnvTrainWithFixedCam,
    #     n_envs=batch_spec.B,
    #     TrajInfoClass=TrajInfoClass,
    #     parallel=parallel,
    # )

    # Build sample tree & obs/action spaces
    ...
    # ===== Per-perturbation Eval Samplers (MultiEval) =====
    callbacks = []
    ...
    for name, cam_cfg in EVAL_CAMERAS.items():
        # env_cls = EnvEvalWithCamCfg(cam_cfg)
        ...
        step_transforms = []
        if name == "nominal":  # record only one (change if you want more)
        ...
        eval_samplers[name] = EvalSampler(
            ...
            envs=cages_i,
            agent=None,  # set below after agent is created; this is fine or set now.
            sample_tree=eval_sample_tree,
    ...
    # Now that agent exists, attach it to each eval sampler:
