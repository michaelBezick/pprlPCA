...
from parllel.samplers.eval import EvalSampler, MultiEvalSampler, SingleEnvMultiCamSampler
...
def build(config: DictConfig) -> Iterator[RLRunner]:
    ...
    with open_dict(config.env.create_scene_kwargs):
        config.env.create_scene_kwargs["camera_configs"] = [
            _to_py(handcrafted["nominal"])   # any placeholder
    ]
    ...
    # with open_dict(config.env):
    #     if vfov is not None:
    #         config.env.create_scene_kwargs.camera_config.vertical_field_of_view = vfov    # override default 62
    #     else:
    ...
    single_cam_sampler = SingleEnvMultiCamSampler(
        camera_dict = EVAL_CAMERAS,
...
# ──────────────────────────────────────────────────────
    ...
    runner = RLRunner(
        ...
        algorithm=algorithm,
        batch_spec=batch_spec,
        eval_sampler=single_cam_sampler,
